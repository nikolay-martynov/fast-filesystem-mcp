# Local Modifications

This is a personal fork of [fast-filesystem-mcp](https://github.com/efforthye/fast-filesystem-mcp) with custom modifications.

See git history for details (`git diff upstream/main`).

## Changes

- Add `newline` parameter to write/edit/extract tools (`auto`/`exact` mode) for proper CRLF handling on Windows: auto mode normalizes `\n` → `\r\n` on writes and matches `\n` in old_text against `\r\n` in files for edits
- Add ZIP compress/extract support via `archiver` and `unzipper` (replaces "use tar instead" errors)
- Reduce default read chunk size from 900KB to ~20KB (5k tokens) for better LLM context management
- Fix regex search: remove `g` flag to avoid `lastIndex` issues with `.test()`
- Add ripgrep-based fast filename search (`rg --files` + filter) with 30s timeout on manual walk fallback
- Add file metadata (size, dates, permissions) in search results
- Fix auto-chunking continuation tokens: store/restore `line_count`, fix null-coalescing (`||`→`??`), detect line/byte mode from token state, incremental reads
- Add relative path resolution via `--relative_paths_base <dir>` CLI flag: relative paths are resolved against this directory instead of `process.cwd()`
- Add `include_hidden` parameter to `fast_search_files`: when `true`, search recurses into hidden directories (`.git`, `.continue`, etc.); default `false` keeps existing behavior of skipping hidden dirs during recursion
- Fix `line_content` in `fast_search_code` and `fast_search_files` (content_search=true) when ripgrep is used: previously returned only the submatch text (e.g. `"Plan2026"`), now returns the full file line as in the non-ripgrep fallback path. The matching submatch is still available via `match_start`/`match_end` for highlighting. Also fixes `formatted_output` which was effectively empty because `line.substring(0, match_start)` on a short submatch produced nothing.